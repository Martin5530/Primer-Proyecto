  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
                ...
                $kira = "";
                if(isset($Row[4])) {
                    $kira = mysqli_real_escape_string($con,$Row[4]);
                } 
                $numeros = "";
                if(isset($Row[5])) {
                    $numeros = mysqli_real_escape_string($con,$Row[5]);
                }
                
                if (!empty($nombres) || !empty($cargo) || !empty($celular) || !empty($descripcion)|| !empty($kira)|| !empty($numeros)) {
                    $query = "insert into tbl_productos(nombres,cargo, celular, descripcion,kira,numeros) values('".$nombres."','".$cargo."','".$celular."','".$descripcion."','".$kira."','".$numeros."')";
